...
<body>
	...
	<div>
		<div style="font-size: 16pt;">
			'<%=vo.getAppName()%>'이 당신의 계정 정보에 접근하는 것을 허용하시겠습니까?
		</div>
		<hr>
		<form id="f1" method="post" action="authorize">
			<input type="hidden" id="oauth_token" name="oauth_token"
				value="<%=vo.getRequestToken()%>" /> <input type="hidden"
				id="allow_deny" name="allow_deny" value="allow" />
			<%
			...
			%>
			<div>
				<%=usersVO.getUserid()%>(<%=usersVO.getUsername()%>) 님 로그인 중!!
			</div>
			<br />
			<br />
			...
			%>
			<input type="button" value="애플리케이션 승인" id="allow"
				onclick="goAllow();"> <input type="button" value="승인 거부"
				id="deny" onclick="goDeny();">
		</form>
	</div>
